      ...
      <div class="grid grid-cols-1 lg:grid-cols-2">
        ...
        <div class="p-5">
          <div class="flex items-start justify-between gap-3 mb-2">
            ...
            <button type="button" class="btn btn-ghost btn-icon shrink-0" @click="close" aria-label="<?php echo e(__('messages.Close')); ?>">
...
<?php endif; ?>
          ...
          <div id="qvVariantWrap" class="hidden mb-4">
            <div class="form-label"><?php echo e(__('messages.ChooseVariant')); ?></div>
          ...
          <button type="button" id="qvAddBtn" class="btn btn-primary btn-block">
            <?php if (isset($component)) { $__componentOriginal20d0f02a197ba5f09810c743e2b069f7 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal20d0f02a197ba5f09810c743e2b069f7 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.store.icon','data' => ['name' => 'cart','class' => 'w-4 h-4']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('store.icon'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
...
<?php $component = $__componentOriginal20d0f02a197ba5f09810c743e2b069f7; ?>
<?php unset($__componentOriginal20d0f02a197ba5f09810c743e2b069f7); ?>
<?php endif; ?><?php echo e(__('messages.AddToCart')); ?>
  ...
  <div x-show="isOpen" x-cloak class="fixed inset-0 z-[70] flex items-center justify-center p-4">
    ...
    <div x-show="isOpen"
      ...
      <div class="dialog-header">
        <h5 class="dialog-title"><?php echo e(__('messages.ChooseVariant')); ?></h5>
...
<script>
(function(){
  const NOIMG = <?php echo json_encode(global_asset(upload_path('products') . '/no-image.png'), 15, 512) ?>;
  const CURRENCY = <?php echo json_encode($currency, 15, 512) ?>;
  const ALLOW_OVERSELLING = window.__ALLOW_OVERSELLING__ !== false;
  const OUT_OF_STOCK_MSG = <?php echo json_encode(__('messages.OutOfStock'), 15, 512) ?>;
  const PREORDER_MSG = <?php echo json_encode(__('messages.PreOrderNow'), 15, 512) ?>;
  const ADD_MSG = <?php echo json_encode(__('messages.AddToCart'), 15, 512) ?>;
  const SELECT_VARIANT_MSG = <?php echo json_encode(__('messages.SelectVariant'), 15, 512) ?>;
  ...
  document.addEventListener("DOMContentLoaded", function(){
    ...
    form.addEventListener("submit", async function(e){
      ...
      try {
        const resp = await fetch(<?php echo json_encode(route('newsletter.subscribe'), 15, 512) ?>, {
          method: "POST",
          headers: {
        ...
        if (resp.ok) {
          ...
          msg.textContent = <?php echo json_encode(__('messages.NewsletterThanks'), 15, 512) ?>;
          emailInput.value = "";
        } else {
          msg.className = "text-danger text-sm mt-2";
          msg.textContent = data.message || <?php echo json_encode(__('messages.NewsletterFailed'), 15, 512) ?>;
        }
      } catch (err) {
        msg.className = "text-danger text-sm mt-2";
        msg.textContent = <?php echo json_encode(__('messages.NetworkErrorTryAgain'), 15, 512) ?>;
      } finally {
        btn.disabled = false;
...
<?php /**PATH /home/quantrocousr/public_html/resources/views/store/partials/home-modals-scripts.blade.php ENDPATH**/ ?>