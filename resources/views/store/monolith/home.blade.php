...
<div class="theme-monolith" style="background:var(--mono-paper); color:var(--mono-ink);">
  ...
  <section class="max-w-7xl mx-auto px-6 md:px-8 pt-12 pb-16">
    <div class="grid md:grid-cols-2 gap-10 items-center">
      <div>
        ...
        <h1 class="m-serif" style="font-weight:480; font-size:clamp(2.6rem,6vw,4.6rem); line-height:1.02; color:var(--mono-ink);">{{ $s->hero_title ?? 'Monolith_HeroTitle' }}</h1>
        <p class="text-sm mt-6 max-w-sm" style="color:var(--mono-moss);">{{ $s->hero_subtitle ?? 'Monolith_HeroSubtitle' }}</p>
        <div class="flex flex-wrap gap-3 mt-8">
          <a href="{{ route('store.shop') }}" class="m-btn m-btn-solid">{{ 'Monolith_ViewTheIndex' }}</a>
          <a href="{{ route('store.contact') }}" class="m-btn">{{ 'Monolith_VisitTheStudio' }}</a>
        </div>
      </div>
      <div class="relative" style="padding-bottom:1.5rem;">
        <div style="border-radius:8px; overflow:hidden; aspect-ratio:4/5;">
          <img src="https://images.unsplash.com/photo-1611652022419-a9419f74343d?auto=format&fit=crop&w=900&q=80" alt="{{ 'Monolith_AltGoldNecklace' }}" style="width:100%; height:100%; object-fit:cover;">
        </div>
        <div class="hidden md:block absolute -bottom-2 -left-10" style="width:44%; border-radius:8px; overflow:hidden; aspect-ratio:1; box-shadow:0 20px 40px -12px rgba(35,48,31,.35); border:6px solid var(--mono-paper);">
          <img src="https://images.unsplash.com/photo-1611652022419-a9419f74343d?auto=format&fit=crop&w=700&q=80" alt="{{ 'Monolith_AltRingDetail' }}" style="width:100%; height:100%; object-fit:cover;">
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="py-14">
    <div class="max-w-[100rem] mx-auto">
      <div class="flex items-baseline justify-between px-6 md:px-8 mb-6">
        <p class="m-label" style="color:var(--mono-moss);">{{ 'Monolith_Departments' }}</p>
        <a href="{{ route('store.shop') }}" class="m-label hover:opacity-60 transition" style="color:var(--mono-rust);">{{ 'Monolith_All' }} →</a>
      </div>
      ...
      <div class="m-mosaic {{ $cats->isEmpty() ? 'm-mosaic-empty' : '' }}">
        @forelse($cats as $i => $cat)
          <a href="{{ route('store.shop', ['category' => $cat->id]) }}" class="m-tile">
            ...
            @else
              @php
                $fallbacks = ['1611652022419-a9419f74343d','1605100804763-247f67b3557e','1611652022419-a9419f74343d','1616401784845-180882ba9ba8'];
              @endphp
        ...
        @empty
          <div class="m-tile" style="grid-area: a;">
            ...
            <span class="m-tile-cap">{{ 'Monolith_TheIndex' }}</span>
          </div>
        @endforelse
      </div>
    </div>
  ...
  <section class="relative" style="height:22rem; overflow:hidden;">
    ...
    <div class="relative h-full flex items-center justify-center px-6 text-center">
      <p class="m-serif" style="font-style:italic; font-size:clamp(1.4rem,3.5vw,2.4rem); color:var(--mono-paper); max-width:38rem;">"{{ $s->footer_text ?? 'Monolith_FooterQuote' }}"</p>
    </div>
  </section>

  <!-- ============ CATALOG LISTING — numbered rows, not a card grid (real products) ============ -->
  <section class="max-w-4xl mx-auto px-6 md:px-8 py-14">
    <p class="m-label mb-2" style="color:var(--mono-moss);">{{ 'Monolith_TheCollection' }}</p>
    <p class="m-serif" style="font-size:2.2rem; color:var(--mono-ink);">{{ 'Monolith_RecentlyCatalogued' }}</p>
    <div class="mt-8">
      ...
      @empty
        <p class="text-sm" style="color:var(--mono-moss);">{{ 'Monolith_IndexEmpty' }}</p>
      @endforelse
    </div>
    <div class="mt-10">
      <a href="{{ route('store.shop') }}" class="m-btn m-btn-solid">{{ 'Monolith_ViewFullIndex' }}</a>
    </div>
  </section>

  <!-- ============ ATELIER GALLERY — 3-image strip, more content ============ -->
  <section class="max-w-7xl mx-auto px-6 md:px-8 pb-14">
    <p class="m-label mb-6" style="color:var(--mono-moss);">{{ 'Monolith_FromTheStudio' }}</p>
    <div class="grid md:grid-cols-3 gap-4">
      <div style="border-radius:8px; overflow:hidden; aspect-ratio:3/4;">
        <img src="https://images.unsplash.com/photo-1616401784845-180882ba9ba8?auto=format&fit=crop&w=700&q=80" alt="{{ 'Monolith_AltHandFinishing' }}" style="width:100%; height:100%; object-fit:cover;">
      </div>
      <div style="border-radius:8px; overflow:hidden; aspect-ratio:3/4;">
        <img src="https://images.unsplash.com/photo-1611591437281-460bfbe1220a?auto=format&fit=crop&w=700&q=80" alt="{{ 'Monolith_AltWorkbench' }}" style="width:100%; height:100%; object-fit:cover;">
      </div>
      <div style="border-radius:8px; overflow:hidden; aspect-ratio:3/4;">
        <img src="https://images.unsplash.com/photo-1605100804763-247f67b3557e?auto=format&fit=crop&w=700&q=80" alt="{{ 'Monolith_AltFinishedRing' }}" style="width:100%; height:100%; object-fit:cover;">
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 md:px-8 pb-16" style="border-top:1px solid rgba(35,48,31,.1); padding-top:3.5rem;">
    <div class="flex items-end justify-between mb-6">
      <p class="m-serif" style="font-size:2rem; color:var(--mono-ink);">{{ 'Monolith_MoreFromTheIndex' }}</p>
      <a href="{{ route('store.shop') }}" class="m-label hover:opacity-60 transition" style="color:var(--mono-rust);">{{ 'ViewAll' }} →</a>
    </div>
    <div class="grid md:grid-cols-3 gap-4">
