...
<section class="relative border-b border-white/[.06] overflow-hidden">
  <div class="absolute -top-24 -right-24 w-80 h-80 bg-brand-violet/25 rounded-full blur-[100px]"></div>
  <div class="relative max-w-7xl mx-auto px-4 py-10">
    <p class="eyebrow text-[10px] text-brand-mint font-bold mb-2">THE FULL DROP</p>
    <h1 class="font-display font-extrabold text-3xl md:text-4xl text-white">Shop All Jewelry</h1>
    <div class="text-sm text-white/50 mt-2">
      {{ trans_choice('messages.products', $total, ['count' => $total]) }}
      @if($hasFilters) · <span class="text-brand-magenta font-semibold">{{ __('messages.FiltersApplied') }}</span>@endif
    </div>
    ...
    <form method="get" action="{{ route('store.shop') }}" class="flex items-end gap-2 flex-wrap mt-4">
      ...
      <button class="bg-gradient-to-r from-brand-magenta to-brand-violet text-white font-bold px-5 py-2 rounded-full text-xs" type="submit">Update</button>
      <button class="border border-white/20 text-white px-5 py-2 rounded-full text-xs lg:hidden" type="button" @click="window.StoreUI.open('filtersDrawer')">Filters</button>
    </form>
  </div>
</section>
