...
<section class="max-w-7xl mx-auto px-4 md:px-8 pt-14 pb-8">
  <div class="border-b border-draft-ink pb-6">
    <p class="font-draft-mono text-xs text-draft-blue font-bold">//RECORD_CATALOG_INDEX</p>
    <h1 class="font-draft-mono text-2xl md:text-3xl font-bold uppercase text-draft-ink">Atelier Specification Index</h1>
    <div class="text-xs font-draft-mono text-draft-grey mt-2">
      MATCHING_LOTS_FOUND: [{{ $total }}]
      @if($hasFilters) · <span class="text-draft-blue font-semibold">[FILTERS_ENGAGED]</span>@endif
    </div>
  </div>
  ...
  <form method="get" action="{{ route('store.shop') }}" class="flex items-center gap-2 flex-wrap mt-5 font-draft-mono text-xs">
    ...
    <button class="draft-btn py-1.5 px-5 text-[10px]" type="submit">[RE_QUERY]</button>
    <button class="draft-btn py-1.5 px-5 text-[10px] lg:hidden" type="button" @click="window.StoreUI.open('filtersDrawer')">[OPEN_FILTERS]</button>
  </form>
</section>
  ...
  <div class="grid lg:grid-cols-[240px_1fr] gap-10">
    <aside class="hidden lg:block">
      <div class="p-4 bg-draft-white border border-draft-border font-draft-mono text-xs">
        <div class="mb-4 text-xs font-bold text-draft-ink">//FILTER_CONTROLS</div>
    ...
    <main>
      ...
      @if($hasFilters)
        <div class="flex flex-wrap gap-2 mb-6 font-draft-mono text-xs">
          ...
          @endif
          <a href="{{ route('store.shop') }}" class="border border-red-500 text-red-600 px-3 py-1 bg-red-50/50">[RESET_ALL]</a>
        </div>
      @endif
            ...
            <nav aria-label="Product pagination">
              <ul class="flex items-center gap-1 font-draft-mono text-xs">
                @if ($products->onFirstPage())
                  <li><span class="px-3 py-1.5 border border-draft-border text-draft-grey">[PREV]</span></li>
                @else
                  <li><a class="px-3 py-1.5 border border-draft-border hover:bg-draft-ink hover:text-white transition" href="{{ $products->previousPageUrl() }}">[PREV]</a></li>
                @endif
                ...
                @if ($products->hasMorePages())
                  <li><a class="px-3 py-1.5 border border-draft-border hover:bg-draft-ink hover:text-white transition" href="{{ $products->nextPageUrl() }}">[NEXT]</a></li>
                @else
                  <li><span class="px-3 py-1.5 border border-draft-border text-draft-grey">[NEXT]</span></li>
                @endif
              </ul>
            </nav>
          </div>
      ...
      @else
        <div class="text-center py-16 border border-draft-border bg-draft-white">
          <p class="font-draft-mono text-sm text-draft-blue mb-3">//NO_LOTS_MATCHED</p>
          <p class="text-xs text-draft-grey mb-4 font-draft-mono">The database returned empty matching lists. Adjust search/filters.</p>
          <a href="{{ route('store.shop') }}" class="draft-btn text-xs py-1.5 px-6">[RESET_QUERY]</a>
        </div>
      @endif
    </main>
  </div>
...
<div x-data="drawer()" x-cloak id="filtersDrawer">
  ...
  <aside class="drawer-panel drawer-end bg-draft-white border-l border-draft-ink text-draft-ink" x-show="isOpen"
    ...
    <div class="drawer-header border-b border-draft-border py-4 px-6 flex justify-between items-center font-draft-mono">
      <h5 class="text-sm m-0 font-bold">//FILTERS_DRAWER</h5>
