...
<section class="bg-brand-paper border-b border-brand-ink/[.06]">
  <div class="max-w-7xl mx-auto px-4 py-10">
    <p class="eyebrow text-[11px] text-brand-sageDark font-bold mb-2">THE FULL COLLECTION</p>
    <h1 class="font-display font-extrabold text-3xl md:text-4xl text-brand-ink">Shop All Jewelry</h1>
    <div class="text-sm text-brand-ink/50 mt-2">
      {{ trans_choice('messages.products', $total, ['count' => $total]) }}
      @if($hasFilters) · <span class="text-brand-sageDark font-semibold">{{ __('messages.FiltersApplied') }}</span>@endif
    </div>
    ...
    <form method="get" action="{{ route('store.shop') }}" class="flex items-end gap-2 flex-wrap mt-4">
      ...
      <button class="bg-brand-sage hover:bg-brand-sageDark transition text-white font-semibold px-5 py-2 rounded-lg text-xs" type="submit">Update</button>
