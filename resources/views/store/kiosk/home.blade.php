...
<div class="theme-kiosk" style="background:#fff; color:#0A0A0A;">
  ...
  <section class="k-hero">
    <div class="max-w-7xl mx-auto relative grid md:grid-cols-[1.2fr_.8fr] gap-8 items-center">
      <div>
        ...
        <h1 class="k-hero-title mt-4">{{ $s->hero_title ?? 'Kiosk_HeroTitle' }}</h1>
        <p class="mt-5 max-w-md text-sm" style="color:#1a1a1a;">{{ $s->hero_subtitle ?? 'Kiosk_HeroSubtitle' }}</p>
        <div class="flex flex-wrap gap-3 mt-8">
          <a href="{{ route('store.shop') }}" class="k-btn">{{ 'Kiosk_ShopTheDrop' }}</a>
          <a href="{{ route('store.contact') }}" class="k-btn k-btn-alt">{{ 'Kiosk_GetOnTheList' }}</a>
        </div>
      </div>
      <div class="hidden md:block relative">
        <div class="k-block" style="aspect-ratio:4/5; overflow:hidden; transform:rotate(2deg); box-shadow:8px 8px 0 var(--kx-black);">
          <img src="https://images.unsplash.com/photo-1620656798579-1984d9e87df7?auto=format&fit=crop&w=800&q=80" alt="{{ 'Kiosk_AltBoldStatementJewelry' }}" style="width:100%; height:100%; object-fit:cover;">
        </div>
        <span class="k-sticker" style="position:absolute; bottom:-1rem; left:-1rem; background:var(--kx-black); transform:rotate(-6deg);">{{ 'Kiosk_Hot' }}</span>
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 py-14">
    <p class="k-label mb-6">{{ 'Kiosk_ShopTheVibe' }}</p>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      @empty
        <p class="col-span-full text-sm">{{ 'Kiosk_NoCategoriesYet' }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 pb-14">
    <div class="flex items-end justify-between mb-6">
      <p class="k-display" style="font-size:2rem;">{{ 'Kiosk_TrendingNow' }}</p>
      <a href="{{ route('store.shop') }}" class="k-label" style="text-decoration:underline;">{{ 'Kiosk_SeeAll' }}</a>
    </div>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      @empty
        <p class="col-span-full text-sm">{{ 'Kiosk_NoProductsYet' }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 pb-14">
    <div class="grid md:grid-cols-2 gap-4">
      <a href="{{ route('store.shop') }}" class="k-block relative flex flex-col justify-end p-8" style="min-height:260px; overflow:hidden;">
        <img src="https://images.unsplash.com/photo-1611652022419-a9419f74343d?auto=format&fit=crop&w=900&q=80" alt="" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;">
        <div style="position:absolute; inset:0; background:linear-gradient(0deg, rgba(233,245,42,.9), rgba(233,245,42,.15));"></div>
        <div class="relative">
          <p class="k-display" style="font-size:2rem;">{{ 'Kiosk_StackItUp' }}</p>
          <span class="k-btn w-fit mt-3" style="display:inline-flex;">{{ 'Kiosk_ShopStacks' }}</span>
        </div>
      </a>
      <a href="{{ route('store.contact') }}" class="k-block relative flex flex-col justify-end p-8" style="min-height:260px; overflow:hidden;">
        ...
        <div class="relative">
          <p class="k-display" style="font-size:2rem; color:var(--kx-yellow);">{{ 'Kiosk_CustomEngraving' }}</p>
          <span class="k-btn k-btn-alt w-fit mt-3" style="display:inline-flex;">{{ 'Kiosk_StartNow' }}</span>
        </div>
      </a>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 pb-14">
    <p class="k-label mb-6">{{ 'Kiosk_BehindTheDrop' }}</p>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      @php $gallery = ['1620656798579-1984d9e87df7','1611652022419-a9419f74343d','1587467512961-120760940315','1522312346375-d1a52e2b99b3']; @endphp
  ...
  <section class="max-w-7xl mx-auto px-6 pb-14">
    <p class="k-display mb-6" style="font-size:2rem;">{{ 'Kiosk_FreshDrops' }}</p>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
  ...
  <section class="k-block-black" style="padding: 2.5rem 1.5rem;">
    <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
      <div><p class="k-display" style="font-size:2rem; color:var(--kx-yellow);">50K+</p><p class="k-label" style="color:#ccc;">{{ 'Kiosk_PiecesShipped' }}</p></div>
      <div><p class="k-display" style="font-size:2rem; color:var(--kx-yellow);">4.8/5</p><p class="k-label" style="color:#ccc;">{{ 'Kiosk_Rating' }}</p></div>
      <div><p class="k-display" style="font-size:2rem; color:var(--kx-yellow);">120K+</p><p class="k-label" style="color:#ccc;">{{ 'Kiosk_Followers' }}</p></div>
      <div><p class="k-display" style="font-size:2rem; color:var(--kx-yellow);">{{ 'Kiosk_Weekly' }}</p><p class="k-label" style="color:#ccc;">{{ 'Kiosk_NewDrops' }}</p></div>
    </div>
  </section>
