...
<section class="px-6 md:px-12 pt-12 pb-8">
  <p class="a-label mb-2">{{ 'Atlas_FullCatalogue' }}</p>
  <h1 style="font-family:'Sora',sans-serif; font-weight:700; font-size:clamp(1.9rem,4vw,3rem);">{{ 'Atlas_ShopAll' }}</h1>
  ...
  <form method="get" action="{{ route('store.shop') }}" class="flex items-end gap-2 flex-wrap mt-5">
    ...
    <button class="a-btn a-btn-outline lg:hidden" type="button" @click="window.StoreUI.open('filtersDrawer')">{{ 'Filters' }}</button>
  </form>
</section>
