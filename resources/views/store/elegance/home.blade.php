...
<div class="theme-elegance bg-white text-[#171310]">
  ...
  <section class="relative h-[92vh] min-h-[560px] max-h-[900px] overflow-hidden">
    ...
    <div class="relative z-10 h-full flex flex-col items-center justify-end text-center px-4 pb-16 md:pb-20">
      <span class="eyebrow text-[11px] text-white/80 font-semibold mb-4">{{ __('messages.Elegance_HeroEyebrow') }}</span>
      ...
      <div class="flex flex-wrap items-center justify-center gap-3">
        <a href="#newin" class="inline-flex items-center gap-2 bg-white hover:bg-gray-100 transition text-black font-semibold px-8 py-3.5 text-sm">{{ __('messages.Elegance_ShopNewIn') }}</a>
        <a href="{{ route('store.shop') }}" class="inline-flex items-center gap-2 bg-transparent border border-white text-white font-semibold px-8 py-3.5 text-sm hover:bg-white/10 transition">{{ __('messages.Elegance_ShopBridal') }}</a>
      </div>
    </div>
    <div class="hidden md:flex absolute top-8 right-8 flex-col items-center justify-center text-center w-24 h-24 rounded-full bg-white/95 text-[#171310] shadow-2xl">
      <span class="text-[9px] font-semibold">{{ __('messages.Elegance_UpTo') }}</span>
      <span class="font-logo text-2xl text-brand-gold leading-none">30%</span>
      <span class="text-[9px] font-semibold">{{ __('messages.Elegance_Off') }}</span>
    </div>
  </section>
  ...
  <section class="border-b border-gray-100">
    <div class="max-w-7xl mx-auto px-4 py-6 grid grid-cols-2 md:grid-cols-4 gap-5">
      <div class="flex items-center gap-3">
        ...
        <div><p class="font-semibold text-xs md:text-sm">{{ __('messages.Elegance_FreeInsuredShipping') }}</p><p class="text-[10px] md:text-xs text-gray-500">{{ __('messages.Elegance_OnAllOrders') }}</p></div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <div><p class="font-semibold text-xs md:text-sm">{{ __('messages.Elegance_CertifiedGemstones') }}</p><p class="text-[10px] md:text-xs text-gray-500">{{ __('messages.Elegance_IgiGiaGraded') }}</p></div>
      </div>
      <div class="flex items-center gap-3">
        ...
        <div><p class="font-semibold text-xs md:text-sm">{{ __('messages.Elegance_EasyReturns') }}</p><p class="text-[10px] md:text-xs text-gray-500">{{ __('messages.Elegance_30DayPolicy') }}</p></div>
      </div>
      <div class="hidden md:flex items-center gap-3">
        ...
        <div><p class="font-semibold text-sm">{{ __('messages.Elegance_StylingConcierge') }}</p><p class="text-xs text-gray-500">{{ __('messages.Elegance_WereHereToHelp') }}</p></div>
      </div>
    </div>
  </section>
  ...
  <section class="border-b border-gray-100 bg-[#FAF8F4]">
    <div class="max-w-7xl mx-auto px-4 py-4 overflow-x-auto">
      <div class="flex items-center gap-8 whitespace-nowrap text-sm">
        <span class="eyebrow text-[10px] text-gray-400 font-semibold shrink-0">{{ __('messages.Elegance_ShopTheEdit') }}</span>
        ...
        @empty
          <span class="text-gray-400 text-sm">{{ __('messages.Elegance_NoCategoriesYet') }}</span>
        @endforelse
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-4 py-12">
    <div class="flex items-end justify-between mb-6">
      <h2 class="font-logo text-2xl md:text-3xl">{{ __('messages.Elegance_TheEditorial') }}</h2>
      <a href="{{ route('store.shop') }}" class="text-xs md:text-sm font-semibold text-brand-gold hover:opacity-70 transition">{{ __('messages.Elegance_ViewTheStory') }}</a>
    </div>
    <div class="grid md:grid-cols-3 gap-3 md:gap-4">
      <a href="{{ route('store.shop') }}" class="group relative md:row-span-2 rounded-lg overflow-hidden aspect-[4/5] md:aspect-auto">
        ...
        <div class="absolute bottom-5 left-5 text-white">
          <p class="eyebrow text-[10px] text-white/70 mb-1">{{ __('messages.Elegance_ChapterOne') }}</p>
          <p class="font-logo text-xl">{{ __('messages.Elegance_LayeredLuminous') }}</p>
        </div>
      </a>
      <a href="{{ route('store.shop') }}" class="group relative rounded-lg overflow-hidden aspect-[4/5] md:aspect-[16/11]">
        ...
        <div class="absolute bottom-4 left-4 text-white">
          <p class="eyebrow text-[10px] text-white/70 mb-1">{{ __('messages.Elegance_ChapterTwo') }}</p>
          <p class="font-logo text-lg">{{ __('messages.Elegance_ThePearlEdit') }}</p>
        </div>
      </a>
      <a href="{{ route('store.shop') }}" class="group relative rounded-lg overflow-hidden aspect-[4/5] md:aspect-[16/11] bg-[#0d0d0d]">
        ...
        <div class="absolute inset-0 bg-gradient-to-t from-black/65 via-black/0 to-transparent"></div>
        <div class="absolute bottom-4 left-4 text-white">
          <p class="eyebrow text-[10px] text-white/70 mb-1">{{ __('messages.Elegance_ChapterThree') }}</p>
          <p class="font-logo text-lg">{{ __('messages.Elegance_StatementGold') }}</p>
        </div>
      </a>
    </div>
  </section>
  ...
  <section id="newin" class="max-w-7xl mx-auto px-4 pb-12">
    <div class="flex items-center justify-between mb-5">
      <h2 class="eyebrow text-sm md:text-base font-bold flex items-center gap-2">{{ __('messages.Elegance_NewInHeading') }} <span class="w-8 h-px bg-brand-gold"></span></h2>
      <a href="{{ route('store.shop') }}" class="text-xs md:text-sm font-semibold text-[#171310] hover:text-brand-gold transition flex items-center gap-1">{{ __('messages.Elegance_ViewAllJewelry') }}
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-5">
      ...
      @empty
        <p class="col-span-full text-gray-400 text-sm">{{ __('messages.Elegance_NoProductsAvailableYet') }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="relative overflow-hidden min-h-[320px] flex items-center">
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-4 py-14 w-full">
      <div class="max-w-sm">
        <p class="eyebrow text-[10px] text-brand-gold font-semibold mb-2">{{ __('messages.Elegance_TheGiftEdit') }}</p>
        <h3 class="font-logo text-3xl mb-3">{{ __('messages.Elegance_GiftingBeautifullyDone') }}</h3>
        <p class="text-sm text-[#5a4a3a] mb-6">{{ __('messages.Elegance_GiftingDescription') }}</p>
        <a href="{{ route('store.shop') }}" class="inline-flex items-center gap-2 bg-brand-black hover:bg-black transition text-white font-semibold px-7 py-3 text-xs eyebrow">{{ __('messages.Elegance_ShopGifts') }}</a>
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-4 py-12">
    <div class="flex items-center justify-between mb-5">
      <h2 class="eyebrow text-sm md:text-base font-bold flex items-center gap-2">{{ __('messages.Elegance_Bestsellers') }} <span class="w-8 h-px bg-brand-gold"></span></h2>
      <a href="{{ route('store.shop') }}" class="text-xs md:text-sm font-semibold text-[#171310] hover:text-brand-gold transition">{{ __('messages.ViewAll') }}</a>
    </div>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-5">
  ...
  <section class="bg-[#FAF8F4] py-14 border-y border-gray-100">
    <div class="max-w-3xl mx-auto px-4 text-center">
      ...
      <p class="font-logo text-2xl md:text-3xl leading-snug mb-4">{{ __('messages.Elegance_PullQuote') }}</p>
      <p class="eyebrow text-[10px] text-gray-400 font-semibold">{{ __('messages.Elegance_StyleJournalAttribution') }}</p>
  ...
  <section class="max-w-7xl mx-auto px-4 py-14">
    <div class="bg-brand-black rounded-2xl p-8 md:p-14 text-center">
      <p class="eyebrow text-[10px] text-brand-gold font-semibold mb-3">{{ __('messages.Elegance_JoinTheEdition') }}</p>
      <h2 class="font-logo text-3xl md:text-4xl text-white mb-3">{{ __('messages.Elegance_BeFirstToKnow') }}</h2>
      <p class="text-sm text-white/60 max-w-md mx-auto mb-6">{{ __('messages.Elegance_NewsletterDescription') }}</p>
      <form action="{{ route('newsletter.subscribe') }}" method="POST" class="flex flex-col sm:flex-row gap-2 max-w-md mx-auto">
        @csrf
        <input type="email" name="email" required placeholder="{{ __('messages.Elegance_EnterEmailAddress') }}" class="flex-1 min-w-0 px-4 py-3 border border-white/15 bg-white/5 outline-none text-sm text-white placeholder:text-white/40" />
        <button type="submit" class="bg-brand-gold hover:bg-brand-goldDark transition text-black font-semibold px-7 py-3 text-xs eyebrow shrink-0">{{ __('messages.Subscribe') }}</button>
      </form>
    </div>
  </section>
