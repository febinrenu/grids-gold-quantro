...
<div class="theme-ledger" style="background:#FAF7F0; color:#1A1E2B;">
  ...
  <section class="l-masthead">
    <div>
      <p class="l-label mb-4">{{ __('messages.Ledger_CatalogueNo', ['year' => now()->format('Y')]) }}</p>
      <h1 class="l-mast-title">{{ $s->hero_title ?? __('messages.Ledger_HeroTitle') }}</h1>
      <p class="text-sm mt-5 max-w-sm" style="opacity:.7;">{{ $s->hero_subtitle ?? __('messages.Ledger_HeroSubtitle') }}</p>
      <a href="{{ route('store.shop') }}" class="l-btn mt-7 w-fit">{{ __('messages.Ledger_ViewCatalogue') }}</a>
    </div>
  ...
  <section class="max-w-6xl mx-auto px-6 md:px-8 py-10 l-rule">
    <p class="l-label mb-5">{{ __('messages.Ledger_IndexOfLots') }}</p>
    <div class="grid md:grid-cols-2 gap-x-12">
      ...
      @empty
        <p class="text-sm" style="opacity:.6;">{{ __('messages.Ledger_CatalogueEmpty') }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="max-w-6xl mx-auto px-6 md:px-8 py-10 l-rule">
    <p class="l-label mb-1">{{ __('messages.Ledger_PlatesOfNote') }}</p>
    <p class="l-serif" style="font-style:italic; font-size:1.7rem; margin-bottom:1.5rem;">{{ __('messages.Ledger_FromTheArchive') }}</p>
    <div class="grid md:grid-cols-3 gap-4">
      <div style="overflow:hidden; aspect-ratio:4/5;">
        <img src="https://images.unsplash.com/photo-1616401784845-180882ba9ba8?auto=format&fit=crop&w=700&q=80" alt="{{ __('messages.Ledger_ProvenanceDetail') }}" style="width:100%; height:100%; object-fit:cover; filter:saturate(.85);">
      </div>
      <div style="overflow:hidden; aspect-ratio:4/5;">
        <img src="https://images.unsplash.com/photo-1587467512961-120760940315?auto=format&fit=crop&w=700&q=80" alt="{{ __('messages.Ledger_GoldDetail') }}" style="width:100%; height:100%; object-fit:cover; filter:saturate(.85);">
      </div>
      <div style="overflow:hidden; aspect-ratio:4/5;">
        <img src="https://images.unsplash.com/photo-1611591437281-460bfbe1220a?auto=format&fit=crop&w=700&q=80" alt="{{ __('messages.Ledger_WorkshopDetail') }}" style="width:100%; height:100%; object-fit:cover; filter:saturate(.85);">
      </div>
    </div>
  </section>
  ...
  <section class="max-w-3xl mx-auto px-6 md:px-8 py-10">
    <p class="l-label mb-1">{{ __('messages.Ledger_FeaturedLots') }}</p>
    <p class="l-serif" style="font-style:italic; font-size:1.9rem;">{{ __('messages.Ledger_NowCataloguing') }}</p>
    <div class="mt-6">
      ...
      @empty
        <p class="text-sm" style="opacity:.6;">{{ __('messages.Ledger_NoLotsYet') }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="max-w-6xl mx-auto px-6 md:px-8 py-10 l-rule">
    <p class="l-label mb-6">{{ __('messages.Ledger_SelectedPlates') }}</p>
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      @php $more = ($products ?? collect())->slice(0, 8); @endphp
      @forelse($more as $p)
        @include('store.partials.product-card', ['p' => $p, 'currency' => $currency])
      @empty
        <p class="col-span-full text-sm" style="opacity:.6;">{{ __('messages.Ledger_NoProductsYet') }}</p>
      @endforelse
    </div>
  </section>
  ...
  <section class="max-w-4xl mx-auto px-6 py-16">
    <div class="grid md:grid-cols-[auto_1fr] gap-6 items-center">
      ...
      <p class="l-serif text-center md:text-left" style="font-style:italic; font-size:clamp(1.3rem,3vw,1.9rem); line-height:1.4;">"{{ $s->footer_text ?? __('messages.Ledger_ProvenanceStatement') }}"</p>
    </div>
  </section>
