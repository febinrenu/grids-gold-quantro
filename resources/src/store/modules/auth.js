...
const state = {
    ...
    price_decimals: 2, // Monetary precision (2 or 3) from database
...
const getters = {
    ...
    getPriceDecimals: state => state.price_decimals,
...
const mutations = {
    ...
    setPriceDecimals(state, priceDecimals) {
        const n = parseInt(priceDecimals, 10);
        state.price_decimals = (n === 3) ? 3 : 2;
    },
...
const actions = {
    async refreshUserPermissions({ commit }, i18n) {
        try {
            ...
            // Set monetary precision from database (2 or 3)
            commit('setPriceDecimals', user.price_decimals || 2);
            // Cache for quick/offline access (e.g. POS) — mirrors cachePriceFormat
            try {
                if (typeof window !== 'undefined' && window.localStorage) {
                    window.localStorage.setItem('app_price_decimals', String((user.price_decimals === 3) ? 3 : 2));
                }
