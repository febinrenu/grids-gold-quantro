              ...
              <template v-for="(detail, idx) in details">
                <tr :key="'r-' + idx">
                  ...
                  <td style="text-align:right; font-weight:600;">
                    {{ formatNumber(detail.quantity, priceDecimals) }} {{ detail.unit }}
                  </td>
                        ...
                        <tbody>
                          <tr v-for="(b, bIdx) in detail.batches" :key="'ab-' + idx + '-' + bIdx" :style="{ background: bIdx % 2 === 1 ? '#f8faff' : '#ffffff', borderTop: '1px solid #e0e7ff' }">
                            ...
                            <td style="padding:6px 10px; text-align:right; color:#1f2937; font-weight:600;">
                              {{ formatNumber(b.qty, priceDecimals) }} {{ detail.unit }}
                            </td>
                          </tr>
                        </tbody>
                      </table>
  ...
  methods: {
    ...
    printTable() {
      ...
      items.forEach(d => {
        ...
        html += `<td style="border:1px solid #ddd; padding:6px; text-align:right;">${this.formatNumber(d.quantity, this.priceDecimals)} ${d.unit || ""}</td>`;
