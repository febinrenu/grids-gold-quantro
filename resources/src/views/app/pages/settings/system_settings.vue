...
{{$t('submit')}}
                  ...
                  <div v-show="activeTab === 'pwa'" class="tab-content">
                    <div class="mb-4">
                      <h5 class="mb-3">{{ $t('PWA_Icons') || 'PWA Icons' }}</h5>
                      <p class="text-muted mb-3">{{ $t('PWA_Icons_Help') || 'These icons are used by the installable PWAs (Admin, Portal, Online Store, Customer Display) on home screens and app launchers. Upload square PNG images; non-square images will be center-cropped.' }}</p>
                      <b-row>
                        <!-- 192x192 Icon -->
                        <b-col lg="6" md="6" sm="12" class="mb-3">
                    ...
                    <b-row class="mt-4">
                      <b-col lg="12" class="d-flex justify-content-end">
                        <b-button variant="primary" size="lg" :disabled="pwaSubmitting" @click="Submit_Pwa_Settings()">
                          {{ pwaSubmitting ? ($t('Saving') || 'Saving...') : $t('submit') }}
                        </b-button>
                      </b-col>
                    </b-row>
                  </div>
                    ...
                    <b-row>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('Enable_3_Decimal_Pricing') || 'Enable 3 Decimal Pricing'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('Enable_3_Decimal_Pricing_Help') || 'When enabled, prices, costs, discounts, taxes and POS totals support up to 3 decimal places (e.g. 0.066) instead of being rounded to 2.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('EnableKitchenDisplay') || 'Enable Kitchen Display'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('EnableKitchenDisplay_Help') || 'When enabled, the POS shows kitchen routing options (Send to Kitchen / Save Without Sending / Send Later) and the Kitchen Display page becomes available to staff.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('Show_Product_GTIN') || 'Show Barcode (GTIN, UPC, EAN, ISBN)'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('Show_Product_GTIN_Help') || 'When enabled, the Barcode (GTIN / UPC / EAN / ISBN) field is shown on the product create form. Turn off to hide it.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('Serial_Numbers') || 'Serial Numbers / IMEI'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('Track_Serial_IMEI_Hint') || 'When enabled, products can opt in to per-unit serial / IMEI tracking, captured at purchase and selected at sale.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('Multi_Pack_Selling') || 'Multi-Pack Selling'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('Enable_Multi_Pack_Selling_Hint') || 'When enabled, products can be sold in multiple quantity packs (e.g. single, 6-pack, case) each with its own price, while inventory stays in one base unit.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                      ...
                      <b-col lg="6" md="6" sm="12" class="mb-3">
                        <b-form-group :label="$t('Offline_Sync') || 'Offline Sync'">
                          ...
                          <p class="text-muted small mt-2 mb-0">
                            {{ $t('Offline_Sync_Help') || 'When enabled, the POS can keep selling without internet and sync sales once back online. When disabled, the POS requires an active connection.' }}
                          </p>
                        </b-form-group>
                      </b-col>
                    </b-row>
                            ...
                            <b-row v-if="setting.logo">
                              <b-col cols="12">
                                ...
                                <div class="invoice-logo-preview-box">
                                  <img
                                    :src="$imgUrl('settings', setting.logo)"
                                    alt="Logo preview"
                                    :style="{
...
export default {
   ...
   ...mapGetters(["currentUser", "getSidebarLayout", "getSideBarToggleProperties"]),
    ...
    tabs() {
      ...
      const baseTabs = [
        ...
        { id: 'pwa', label: this.$t('PWA_Settings') || 'PWA Settings', icon: 'smartphone', description: this.$t('PWA_Settings_Help') || 'Manage the icons used when installing the app on a device.' },
        { id: 'localization', label: this.$t('Localization'), icon: 'globe', description: 'Language, currency, and timezone settings' },
        { id: 'features', label: this.$t('Feature_Toggles') || 'Feature Toggles', icon: 'toggle-right', description: this.$t('Feature_Toggles_Help') || 'Enable or disable optional modules and features.' },
  ...
  methods: {
    ...
    Update_Settings() {
      ...
      axios
        ...
        .then(response => {
          ...
          try {
            const dec = self.setting.enable_3_decimal_pricing ? 3 : 2;
            cachePriceDecimals(dec);
    ...
    Get_Pwa_Settings() {
      axios
        .get("get_pwa_settings")
        .then(response => {
          const data = (response.data && response.data.settings) || {};
          this.pwa_settings.id = data.id || null;
          this.pwa_settings.icon_192_url = data.icon_192_url || "";
          this.pwa_settings.icon_512_url = data.icon_512_url || "";
          this.pwa_settings.icon_192_exists = !!data.icon_192_exists;
          this.pwa_settings.icon_512_exists = !!data.icon_512_exists;
        })
        .catch(() => {
          // Silently fail
        });
    ...
    async onPwaIcon192Selected(e) {
      const file = e.target.files[0];
      if (!file) { this.pwa_settings.icon_192 = ""; return; }
      try {
        const { valid } = await this.$refs.PwaIcon192.validate(e);
        this.pwa_settings.icon_192 = valid ? file : "";
      } catch (err) {
        this.pwa_settings.icon_192 = file;
      }
    },

    async onPwaIcon512Selected(e) {
      const file = e.target.files[0];
      if (!file) { this.pwa_settings.icon_512 = ""; return; }
      try {
        const { valid } = await this.$refs.PwaIcon512.validate(e);
        this.pwa_settings.icon_512 = valid ? file : "";
      } catch (err) {
        this.pwa_settings.icon_512 = file;
      }
    },
    ...
    Submit_Pwa_Settings() {
      ...
      axios
        .post("update_pwa_settings", formData)
        .then(() => {
          this.makeToast("success", this.$t("Successfully_Updated"), this.$t("Success"));
          this.pwa_settings.icon_192 = "";
          this.pwa_settings.icon_512 = "";
    ...
    Get_Settings() {
      axios
        ...
        .then(response => {
          ...
          try {
            const dec = this.setting.enable_3_decimal_pricing ? 3 : 2;
            cachePriceDecimals(dec);
            this.$store.commit('setPriceDecimals', dec);
          } catch (e) {}
