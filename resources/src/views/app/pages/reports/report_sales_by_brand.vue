<template>
  ...
  export default {
    ...
    sumCount(rowObj) {
      ...
      return sum.toFixed(this.priceDecimals) + ' ' + this.currency;
    },
    ...
    printTableOnly() {
      ...
      reportsData.forEach(row => {
        ...
        this.columns.forEach(col => {
          ...
          if (col.field === 'total_sales') {
            ...
            cellContent = `${parseFloat(row.total_sales || 0).toFixed(this.priceDecimals)} ${this.currency || ''}`;
          }
      ...
      tableHtml += `<td class="text-left font-weight-bold">${totalSales.toFixed(this.priceDecimals)} ${this.currency || ''}</td>`;
      tableHtml += `</tr></tfoot>`;
