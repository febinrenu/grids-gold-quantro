    ...
    async exportPDF(){
      try{
        ...
        const body = items.map(r => ([
          ...
          r.warehouse || '',
          r.user || '',
          this.money ? this.money(Number(r.GrandTotal||0)) : Number(r.GrandTotal||0).toFixed(this.priceDecimals),
          this.money ? this.money(Number(r.TaxNet||0))     : Number(r.TaxNet||0).toFixed(this.priceDecimals),
          this.money ? this.money(Number(r.discount||0))   : Number(r.discount||0).toFixed(this.priceDecimals),
          this.money ? this.money(Number(r.shipping||0))   : Number(r.shipping||0).toFixed(this.priceDecimals),
          String(r.age_days ?? '')
        ]));
        ...
        autoTable(doc, {
          ...
          foot: [[
            ...
            { content: this.money ? this.money(tAmount)   : tAmount.toFixed(this.priceDecimals),   styles:{ halign:'right', fontStyle:'bold' } },
            { content: this.money ? this.money(tTax)      : tTax.toFixed(this.priceDecimals),      styles:{ halign:'right', fontStyle:'bold' } },
            { content: this.money ? this.money(tDiscount) : tDiscount.toFixed(this.priceDecimals), styles:{ halign:'right', fontStyle:'bold' } },
            { content: this.money ? this.money(tShip)     : tShip.toFixed(this.priceDecimals),     styles:{ halign:'right', fontStyle:'bold' } }
          ]],
