          ...
          <table class="table table-hover table-sm vgt-table mb-0">
            ...
            <tbody>
              <tr v-for="(t, idx) in filteredTransactions" :key="idx">
                ...
                <td style="text-align:right;">
                  <span v-if="t.unit_value !== null">{{ currencySymbol }} {{ formatNumber(t.unit_value, priceDecimals) }}</span>
                  <span v-else class="text-muted">—</span>
                </td>
  ...
  methods: {
    ...
    printTable() {
      ...
      items.forEach(r => {
        ...
        html += `<td style="border:1px solid #ddd; padding:6px; text-align:right;">${r.unit_value !== null ? this.formatNumber(r.unit_value, this.priceDecimals) : '—'}</td>`;
