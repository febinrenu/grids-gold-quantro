        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field === 'unit_cost'">
            <span v-if="props.row.unit_cost !== null">
              {{ currencySymbol }} {{ formatNumber(props.row.unit_cost, priceDecimals) }}
            </span>
            <span v-else class="text-muted">—</span>
          </span>
          ...
          <span v-else-if="props.column.field === 'value'">
            <span v-if="props.row.value !== null">
              {{ currencySymbol }} {{ formatNumber(props.row.value, priceDecimals) }}
            </span>
            <span v-else class="text-muted">—</span>
          </span>
  ...
  methods: {
    ...
    printTableOnly() {
      ...
      items.forEach(r => {
        ...
        html += `<td style="border:1px solid #ddd; padding:6px; text-align:right;">${r.unit_cost !== null ? this.formatNumber(r.unit_cost, this.priceDecimals) : '—'}</td>`;
        html += `<td style="border:1px solid #ddd; padding:6px; text-align:right;">${r.value !== null ? this.formatNumber(r.value, this.priceDecimals) : '—'}</td>`;
