                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr
                          ...
                          <td>
                            {{currentUser.currency}} {{formatNumber(detail.Net_price, 3)}}
                            <select
                              ...
                              :title="$t('Pack')"
                            >
                              <option v-for="pack in detail.packs" :key="'pk-'+detail.detail_id+'-'+pack.id" :value="pack.id">
                                {{ pack.name }} (×{{ pack.multiplier }})
                              </option>
                            </select>
                          </td>
                          <td>
    ...
    Update_Sale() {
      if (this.verifiedForm()) {
        ...
        if (this.sale.statut === 'completed') {
          ...
          if (badSerial) {
            this.makeToast('warning', `${this.$t('Serials_Count_Mismatch')} (${badSerial.name})`, this.$t('Warning') || 'Warning');
            return;
          }
        }
