                          ...
                          <td>
                            <div class="d-flex align-items-center">
                              ...
                              <select
                                ...
                                :title="$t('Pack')"
                              >
                                <option v-for="pack in detail.packs" :key="'pk-'+detail.detail_id+'-'+pack.id" :value="pack.id" :disabled="!isOversellingAllowed && detail.product_type !== 'is_service' && Number(pack.multiplier) > 1 && Number(pack.multiplier) > Number(detail.stock)">
                                  {{ pack.name }} (×{{ pack.multiplier }})<template v-if="!isOversellingAllowed && detail.product_type !== 'is_service' && Number(pack.multiplier) > 1 && Number(pack.multiplier) > Number(detail.stock)"> — {{ $t('Out_of_Stock') || 'out of stock' }}</template>
                                </option>
                              </select>
                            </div>
                          </td>
        ...
        <b-form @submit.prevent="submit_Update_Detail">
          <b-row>
           ...
           <b-col lg="6" md="6" sm="12">
            ...
            <!-- Serial / IMEI numbers are selected inline in the order table (serial sub-row). -->
...
export default {
  ...
  methods: {
    ...
    onChangePriceType(detail, newType){
      ...
      const selPack = this.getSelectedPack(detail);
    ...
    applyPackPrice(detail){
      ...
      const unitAfterDiscount = (parseFloat(detail.Unit_price) || 0) - (parseFloat(detail.DiscountNet) || 0);
    ...
    onChangePack(detail){
      ...
      if (!this.isOversellingAllowed && detail.product_type !== 'is_service') {
        ...
        const avail = Number(detail.stock) || 0;
