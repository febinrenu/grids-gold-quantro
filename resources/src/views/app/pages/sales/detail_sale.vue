          ...
          <table class="invoice-products-table invoice-products-desktop">
            ...
            <tbody>
              <tr v-for="(detail, index) in details" :key="index" :class="{'invoice-products-row-even': index % 2 === 1}">
                ...
                <td class="invoice-product-price-cell">
                  {{formatNumber(detail.quantity,2)}} {{ packLineUnit(detail) }}
                  <div v-if="detail.pack_name && Number(detail.pack_multiplier) > 1" style="font-size:11px;color:#6b7280;font-weight:400;">(&times;{{ detail.pack_multiplier }}) = {{ formatNumber(detail.quantity * detail.pack_multiplier, 2) }} {{ detail.unit_sale }}</div>
                </td>
              ...
              <div class="invoice-product-card-body">
                ...
                <div class="invoice-product-card-details">
                  ...
                  <div class="invoice-product-card-detail-item">
                    ...
                    <span class="invoice-product-card-detail-value">{{formatNumber(detail.quantity,2)}} {{ packLineUnit(detail) }}<template v-if="detail.pack_name && Number(detail.pack_multiplier) > 1"> (&times;{{ detail.pack_multiplier }} = {{ formatNumber(detail.quantity * detail.pack_multiplier, 2) }} {{ detail.unit_sale }})</template></span>
                  </div>
                  <div class="invoice-product-card-detail-item">
