...
class WooCommerceSyncController extends BaseController
{
    ...
    // Fixed queue names so a persistent worker / Supervisor can subscribe reliably.
    // Previously jobs were dispatched to dynamic per-sync queues (e.g. woocommerce-sync-33),
    // which no static worker could listen to, causing syncs to hang at "queued_next_batch".
    private const WOO_PRODUCTS_QUEUE = 'woocommerce-sync';
    private const WOO_STOCK_QUEUE = 'woocommerce-stock';
    ...
    public function syncProducts(Request $request)
    {
        ...
        // Fresh sync: drop any cached remote index so the first batch rebuilds it
        // from the current remote catalog (later batches reuse the fresh copy).
        SyncService::forgetRemoteIndexCache($settings);
    ...
    public function stopStockSync(Request $request)
    {
        ...
        try {
            if (Schema::hasTable('jobs')) {
                $queue = (string) ($state['queue'] ?? self::WOO_STOCK_QUEUE);
    ...
    public function resetSync(Request $request)
    {
        ...
        // Mappings are being cleared; a cached remote index would now be stale and
        // could cause duplicate remote products on the next sync.
        $resetSettings = WooCommerceSetting::first();
        if ($resetSettings) {
            SyncService::forgetRemoteIndexCache($resetSettings);
        }
    ...
    public function resetProductsSync(Request $request)
    {
        ...
        // Mappings are being cleared; invalidate the cached remote index to avoid
        // creating duplicate remote products on the next sync.
        $resetSettings = WooCommerceSetting::first();
        if ($resetSettings) {
            SyncService::forgetRemoteIndexCache($resetSettings);
        }
