...
class SettingsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('logo') && $request->file('logo')->isValid()) {
            ...
            $path = upload_public_path('settings');
            ...
            $image_resize = Image::make($image->getRealPath());
             ...
             $image_resize->resize(800, 800, function ($constraint) {
                ...
                $constraint->upsize();
            })->save(upload_public_path('settings/'.$filename));
        ...
        Setting::whereId($id)->update([
            ...
            // Monetary precision toggle: 3 decimals when enabled, 2 otherwise
            ...
            // Show/hide the Barcode (GTIN / UPC / EAN / ISBN) field on the product create form (default off)
            'show_product_gtin' => $request->has('show_product_gtin')
                ...
                : (int) ($setting->show_product_gtin ?? 0),
    ...
    public function get_Settings_data_api(Request $request)
    {
        ...
        if ($settings) {
            ...
            // Monetary precision toggle (3-decimal pricing)
            $item['enable_3_decimal_pricing'] = (bool) ($settings->enable_3_decimal_pricing ?? false);
            // Show product GTIN/barcode field on the product create form (default false)
            $item['show_product_gtin'] = (bool) ($settings->show_product_gtin ?? false);
    ...
    public function update_appearance_settings(Request $request, $id)
    {
        ...
        if ($request->hasFile('favicon') && $request->file('favicon')->isValid()) {
            ...
            if (in_array($extension, ['ico', 'png'])) {
                ...
                $favicon->move(upload_public_path('settings'), $faviconFilename);

                // Delete old favicon if it exists and is not default
                if ($currentFavicon && $currentFavicon !== 'favicon.ico') {
                    $oldFaviconPath = upload_public_path('settings/'.$currentFavicon);
     ...
     * data. We do not store PWA icon filenames in the DB — icons live in the
     * tenant's own directory at images/tenants/{id}/pwa/pwa-icon-{192,512}.png.
     */
    public function get_pwa_settings(Request $request)
    {
        ...
        $iconDir   = public_path(upload_path('pwa'));
        ...
        // When the tenant has no icon yet (missing images/tenants/{id}/pwa),
        // pwa_icon_url() returns the shared /pwa_images default so the preview
        // still shows a valid image instead of a broken/404 one.
        return response()->json([
            'settings' => [
                'id' => $setting ? $setting->id : null,
                'icon_192_url' => pwa_icon_url(192),
                'icon_512_url' => pwa_icon_url(512),
    ...
    /**
     * Replace this tenant's PWA icons used by its manifests (admin, portal,
     * store, customer-display). Accepts up to two image uploads: icon_192 and
     * icon_512. Each uploaded image is resized to its target square size and
     * saved as images/tenants/{id}/pwa/pwa-icon-{192,512}.png in the tenant's
     * own directory, overwriting the previous file.
     */
    public function update_pwa_settings(Request $request)
    {
        ...
        // Ensure this tenant's PWA folder exists (images/tenants/{id}/pwa).
        // New tenants may not have it yet, so create it — including parents —
        // before writing the resized icons.
        $dir = public_path(upload_path('pwa'));
        if (! is_dir($dir) && ! @mkdir($dir, 0755, true) && ! is_dir($dir)) {
            return response()->json([
                'success' => false,
                'message' => 'Could not create the PWA icons directory.',
            ], 500);
        }
