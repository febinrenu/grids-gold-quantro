...
class WooCommerceSync extends Command
{
    ...
    // Fixed queue names (must match the jobs' re-dispatch queues so this command's
    // inline worker drains the next batches instead of watching an empty per-sync queue).
    ...
    public function handle(): int
    {
        ...
        // Fresh sync run: drop any cached remote index so the first batch rebuilds it.
        \App\Services\WooCommerce\SyncService::forgetRemoteIndexCache($settings);
