...
class SyncService
{
     ...
     */
    /**
     * Invalidate the cached remote product index for a given store.
     *
     * Must be called when a fresh sync starts or when local Woo mappings are reset,
     * so a stale index can never cause duplicate remote products to be created.
     */
    public static function forgetRemoteIndexCache(WooCommerceSetting $settings): void
    {
        try {
            $client = new Client($settings->store_url, $settings->consumer_key, $settings->consumer_secret);
            \Illuminate\Support\Facades\Cache::store('file')->forget('woo_remote_index:'.$client->fingerprint());
        } catch (\Throwable $e) {
            // Best-effort: cache invalidation must never break the sync.
        }
    }

    /**
     * Return the remote product index, reusing a cached copy across sync batches.
     *
     * buildRemoteIndex() pages the entire remote catalog; with small per-batch sizes
     * (default 5 products) rebuilding it every batch re-downloads the whole store many
     * times, which is slow and can trip stuck-detection / Woo API rate limits.
     *
     * Caching is safe because each batch processes a distinct slice of LOCAL products
     * (cursor = id > last_product_id), so an index of pre-existing remote products does
     * not need to reflect rows we just created earlier in the same run.
     *
     * Set WOO_REMOTE_INDEX_TTL=0 to disable caching (always rebuild).
     */
    private function getRemoteIndexCached(?callable $progress = null): array
    {
        $ttl = (int) env('WOO_REMOTE_INDEX_TTL', 900);
        if ($ttl <= 0) {
            return $this->buildRemoteIndex($progress);
        }
        $ttl = min($ttl, 3600);

        $cache = \Illuminate\Support\Facades\Cache::store('file');
        $key = 'woo_remote_index:'.$this->client->fingerprint();

        $cached = $cache->get($key);
        if (is_array($cached)
            && array_key_exists('by_sku', $cached)
            && array_key_exists('by_external_id', $cached)) {
            if ($progress) {
                $progress(['stage' => 'index_remote', 'remote_cached' => true]);
            }
            return [$cached['by_sku'], $cached['by_external_id']];
        }

        [$remoteBySku, $remoteByExternalId] = $this->buildRemoteIndex($progress);

        // Only cache a non-empty index (an empty result usually means the remote
        // listing call failed; we don't want to pin that for 15 minutes).
        if (!empty($remoteBySku) || !empty($remoteByExternalId)) {
            $cache->put($key, [
                'by_sku' => $remoteBySku,
                'by_external_id' => $remoteByExternalId,
            ], $ttl);
        }
        ...
        while ($page <= $cap) {
            ...
            foreach ($items as $v) {
                ...
                DB::transaction(function () use ($product, $varWooId, $sku, $variantName, $priceF, $defaultWarehouseId, $v) {
                    ...
                    // Barcode (GTIN / UPC / EAN / ISBN) from Woo variation Global Unique ID (WooCommerce 9.2+).
                    if (array_key_exists('global_unique_id', $v)) {
                        $vGtin = trim((string) ($v['global_unique_id'] ?? ''));
                        $pv->gtin = $vGtin !== '' ? $vGtin : null;
                    }
            ...
            for ($i = $indexInPage; $i < $countItems; $i++) {
                ...
                try {
                    ...
                    DB::transaction(function () use (
                        ...
                        // Barcode (GTIN / UPC / EAN / ISBN) from Woo Global Unique ID (WooCommerce 9.2+).
                        // Only touch when Woo actually returns the field, so older stores don't wipe it.
                        if (array_key_exists('global_unique_id', $wp)) {
                            $gtinVal = trim((string) ($wp['global_unique_id'] ?? ''));
                            $product->gtin = $gtinVal !== '' ? $gtinVal : null;
                        }
    ...
    ): array
    {
        ...
        // The index pages the ENTIRE remote catalog, so rebuilding it on every small
        // batch is very expensive. Cache it per-store across batches of the same sync.
        if ($shouldCancel) {
            $shouldCancel();
        }
        [$remoteBySku, $remoteByExternalId] = $this->getRemoteIndexCached($progress);
        ...
        $query->orderBy('id')->chunk(100, function ($products) use (
            ...
            foreach ($products as $product) {
                ...
                try {
                    ...
                    // Barcode (GTIN / UPC / EAN / ISBN) -> Woo native Global Unique ID (WooCommerce 9.2+).
                    // Always send so an emptied local value clears it remotely; ignored by older Woo.
                    $payload['global_unique_id'] = trim((string) ($product->gtin ?? ''));
            ...
            foreach ($variants as $var) {
                ...
                $localVariants[] = [
                    ...
                    'payload' => [
                        ...
                        // Barcode (GTIN / UPC / EAN / ISBN) -> Woo variation Global Unique ID (WooCommerce 9.2+).
                        'global_unique_id' => trim((string) ($var->gtin ?? '')),
                        'attributes' => [[
                            'name' => 'Variant',
                            'option' => $option,
                        ]],
