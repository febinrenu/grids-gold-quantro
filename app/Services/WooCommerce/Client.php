...
class Client
{
    ...
    /**
     * Stable identifier for the target store + credentials.
     * Used to key per-store caches (e.g. the remote product index) so that
     * consecutive sync batches against the same store can reuse cached data.
     */
    public function fingerprint(): string
    {
        return md5($this->baseUrl.'|'.$this->key);
    }
